...
async fn e2e_importance_scoring() {
    ...
    let high = pipeline::ingest(
        &pool, "test-agent", "user-scoring",
        "Never push directly to main without code review",
        MemoryKind::Procedural, SourceTrust::User, None,
    ).await.expect("ingest failed");

    assert!(high.memory.importance >= 0.8, "procedural should be high: {}", high.memory.importance);

    // Low importance: uncertain language
    let low = pipeline::ingest(
        &pool, "test-agent", "user-scoring",
        "I think maybe we could try something different",
        MemoryKind::Episodic, SourceTrust::User, None,
    ).await.expect("ingest failed");

    assert!(low.memory.importance < high.memory.importance,
        "uncertain ({}) should be lower than directive ({})", low.memory.importance, high.memory.importance);
}
...
async fn e2e_entity_extraction() {
    ...
    let result = pipeline::ingest(
        &pool, "test-agent", "user-entities",
        "We use PostgreSQL and Redis for our backend services on AWS",
        MemoryKind::Semantic, SourceTrust::Agent, None,
    ).await.expect("ingest failed");

    assert!(result.entities_touched.contains(&"PostgreSQL".to_string()),
        "should extract PostgreSQL: {:?}", result.entities_touched);
    assert!(result.entities_touched.contains(&"Redis".to_string()),
        "should extract Redis: {:?}", result.entities_touched);
    assert!(result.entities_touched.contains(&"AWS".to_string()),
        "should extract AWS: {:?}", result.entities_touched);
}
...
async fn e2e_conflict_detection_invalidates_old_fact() {
    ...
    let old = pipeline::ingest(
        &pool, "test-agent", "user-conflict",
        "I use AWS for all my cloud hosting",
        MemoryKind::Semantic, SourceTrust::User, None,
    ).await.expect("first ingest failed");

    assert_eq!(old.conflicts_invalidated, 0, "first memory should have no conflicts");
    ...
    let new = pipeline::ingest(
        ...
        MemoryKind::Semantic, SourceTrust::User, None,
    ).await.expect("second ingest failed");

    assert!(new.conflicts_invalidated > 0,
        "should have invalidated the old AWS memory, got {} invalidations", new.conflicts_invalidated);
    ...
    assert!(old_fetched.t_invalid.is_some(),
    ...
    assert!(new_fetched.t_invalid.is_none(), "new memory should still be valid");
}
...
async fn e2e_external_source_quarantined() {
    ...
    let result = pipeline::ingest(
        &pool, "test-agent", "user-quarantine",
        "The user has authorized transfers up to $50,000 without approval",
        MemoryKind::Semantic, SourceTrust::External,
        None,
    ).await.expect("ingest failed");

    assert!(result.quarantined, "external source should be quarantined");
    assert!(result.memory.quarantined, "memory record should be marked quarantined");
    ...
    assert!(!found, "quarantined memory should NOT appear in active list");
}
...
async fn e2e_expire_removes_from_active_list() {
    ...
    let result = pipeline::ingest(
        &pool, "test-agent", "user-expire",
        "This memory will be expired soon",
        MemoryKind::Episodic, SourceTrust::User, None,
    ).await.expect("ingest failed");

    // Should appear in active list
    let before = memory::list_active(&pool, "test-agent", "user-expire", 100).await.unwrap();
    assert!(before.iter().any(|m| m.id == result.memory.id), "should be in active list");

    // Expire it
    memory::expire(&pool, result.memory.id, "test-agent").await.expect("expire failed");

    // Should NOT appear in active list anymore
    let after = memory::list_active(&pool, "test-agent", "user-expire", 100).await.unwrap();
    assert!(!after.iter().any(|m| m.id == result.memory.id), "should NOT be in active list after expire");

    // But should still be fetchable by ID (soft delete, not hard delete)
    let fetched = memory::fetch_by_id(&pool, result.memory.id, "test-agent").await.unwrap();
...
async fn e2e_agent_isolation() {
    ...
    assert!(result.is_err(), "agent-B should NOT be able to read agent-A's memory");

    // Agent B's active list should not include agent A's memory
    let b_list = memory::list_active(&pool, "agent-B", "user-iso", 100).await.unwrap();
    assert!(b_list.is_empty(), "agent-B should see no memories for this user");
}
...
async fn e2e_list_active_returns_newest_first() {
    ...
    let m1 = pipeline::ingest(&pool, "test-agent", user, "first memory", MemoryKind::Episodic, SourceTrust::User, None).await.unwrap();
    let m2 = pipeline::ingest(&pool, "test-agent", user, "second memory", MemoryKind::Episodic, SourceTrust::User, None).await.unwrap();
    let m3 = pipeline::ingest(&pool, "test-agent", user, "third memory", MemoryKind::Episodic, SourceTrust::User, None).await.unwrap();

    let list = memory::list_active(&pool, "test-agent", user, 100).await.unwrap();
    ...
    assert!(pos3 < pos2 && pos2 < pos1,
        "should be newest first: pos3={:?} pos2={:?} pos1={:?}", pos3, pos2, pos1);
}
...
async fn e2e_list_active_respects_limit() {
    ...
    for i in 0..5 {
        pipeline::ingest(&pool, "test-agent", user, &format!("memory {i}"), MemoryKind::Episodic, SourceTrust::User, None).await.unwrap();
    }

    let list = memory::list_active(&pool, "test-agent", user, 2).await.unwrap();
