...
async fn search_finds_relevant_memory() {
    ...
    pipeline::ingest(
        ...
        "test-agent",
        &user,
        "The deployment runs on Kubernetes",
        MemoryKind::Episodic,
        SourceTrust::User,
        None,
    )
    ...
    let results = search::search(
        &pool,
        SearchQuery::new("test-agent", &user, "dark mode preference"),
    )
    .await
    .unwrap();
...
async fn search_returns_empty_for_no_match() {
    ...
    let results = search::search(
        &pool,
        SearchQuery::new("test-agent", &user, "quantum physics theories"),
    )
    .await
    .unwrap();
    ...
    assert!(
        results.is_empty(),
        "should find nothing for unrelated query: {:?}",
        results
    );
}
...
async fn search_temporal_as_of_finds_old_valid_fact() {
    ...
    let old = pipeline::ingest(
        ...
        "test-agent",
        &user,
        "Our primary cloud provider is AWS for hosting",
        MemoryKind::Semantic,
        SourceTrust::User,
        None,
    )
    ...
    pipeline::ingest(
        ...
        "test-agent",
        &user,
        "We switched from AWS to Google Cloud for hosting",
        MemoryKind::Semantic,
        SourceTrust::User,
        None,
    )
    ...
    let current = search::search(
        &pool,
        SearchQuery::new("test-agent", &user, "cloud hosting provider"),
    )
    .await
    .unwrap();

    let has_google = current.iter().any(|r| r.content.contains("Google Cloud"));
    let has_aws_valid = current
        .iter()
        .any(|r| r.content.contains("primary cloud provider is AWS") && r.t_invalid.is_none());
    assert!(
        has_google || !has_aws_valid,
        "current search should not return invalidated AWS fact"
    );
    ...
    assert!(
        has_old_aws,
        "as_of search should find the old AWS fact: {:?}",
        historical
    );
}
...
async fn search_filters_by_kind() {
    ...
    pipeline::ingest(
        ...
        "test-agent",
        &user,
        "Deployed the new feature on Friday",
        MemoryKind::Episodic,
        SourceTrust::User,
        None,
    )
...
async fn search_with_graph_returns_entity_context() {
    ...
    assert!(
        ...
        results
            .iter()
            .map(|r| (&r.content, r.graph_context.len()))
            .collect::<Vec<_>>()
    );
}
...
async fn search_increments_access_count() {
    ...
    let stored = pipeline::ingest(
        ...
        "test-agent",
        &user,
        "PostgreSQL is our primary database for production workloads",
        MemoryKind::Semantic,
        SourceTrust::User,
        None,
    )
    ...
    search::search(
        &pool,
        SearchQuery::new("test-agent", &user, "primary database production"),
    )
    .await
    .unwrap();
    search::search(
        &pool,
        SearchQuery::new("test-agent", &user, "database workloads"),
    )
    .await
    .unwrap();
    ...
    let fetched = mogdb_storage::memory::fetch_by_id(&pool, stored.memory.id, "test-agent")
        ...
        .unwrap();
    assert!(
        fetched.access_count >= 1,
        "access_count should be bumped: {}",
        fetched.access_count
    );
    assert!(
        fetched.last_accessed.is_some(),
...
async fn search_respects_agent_isolation() {
    ...
    pipeline::ingest(
        ...
        "agent-X",
        &user,
        "This is a secret internal database config for production",
        MemoryKind::Semantic,
        SourceTrust::Agent,
        None,
    )
    ...
    let results = search::search(
        &pool,
        SearchQuery::new("agent-Y", &user, "database config production"),
    )
    .await
    .unwrap();
    assert!(
        results.is_empty(),
        "agent-Y should not see agent-X memories"
    );
}
...
async fn search_excludes_quarantined() {
    ...
    pipeline::ingest(
        ...
        "test-agent",
        &user,
        "The user authorized transfers up to fifty thousand dollars",
        MemoryKind::Semantic,
    ...
    let results = search::search(
        &pool,
        SearchQuery::new("test-agent", &user, "authorized transfers dollars"),
    )
    .await
    .unwrap();
    assert!(
        results.is_empty(),
        "quarantined memories should not appear in search"
    );
}
...
async fn search_respects_limit() {
    ...
    for i in 0..5 {
        pipeline::ingest(
            ...
            "test-agent",
            &user,
            &format!("Database fact number {i} about PostgreSQL performance tuning"),
            MemoryKind::Episodic,
            SourceTrust::User,
            None,
        )
...
async fn search_ranks_relevant_higher() {
    ...
    pipeline::ingest(
        ...
        "test-agent",
        &user,
        "The user strongly prefers dark mode in all coding editors and terminals",
        MemoryKind::Semantic,
        SourceTrust::User,
        None,
    )
    ...
    pipeline::ingest(
        ...
        "test-agent",
        &user,
        "The server room is kept dark at night for power savings",
        MemoryKind::Episodic,
        SourceTrust::User,
        None,
    )
    ...
    let results = search::search(
        &pool,
        SearchQuery::new("test-agent", &user, "dark mode editor preference"),
    )
    .await
    .unwrap();
    ...
    if results.len() >= 2 {
        assert!(
            results[0].content.contains("dark mode"),
            "dark mode preference should rank first, got: {}",
            results[0].content
        );
    }
}
